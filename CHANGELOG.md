...
notice 批量注册 + 三段式结果消息（部分成功语义，作者拍板）。

### Added
- **notice 多进程参数**：`process_id` 接受单个编号或编号列表
  （含 JSON 数组字符串通道防御，解析复用 `parse_process_ids`）。
  **逐个独立校验（部分成功语义）**：有效编号照常注册，不存在/已
  结束的编号单独报告、不整批连坐；**有生效即 success，全失败才
  error**。`interval_seconds` 对整批共享。
- **三段式结果消息**：固定三段「已生效 → 不存在 → 已结束」按序
  拼接、空段跳过，单进程/批量/混合共用同一条生成规则（零特例），
  引导语每类最多出现一次：
  - `已生效：#1、#2（每 900s 进度通知 + 完成通知）`
  - `不存在：#42、#43（用 process_tools_list 查看全部编号）`
  - `已结束：#3「构建」completed exit=0、#4 failed exit=2（用
    process_tools_check 查看结果）`
- 批量全成功压缩为一行枚举；「新注册 vs 幂等更新」合并为
  「已生效」（重复注册本就幂等）；单进程消息同样走新格式（旧
  「进程 #N 已注册通知：…」退役，作者拍板统一）。

### Tests
- 新增 `tests/test_notice_multi.py` 14 例：单/批全成功一行格式、
  去重、JSON 防御、部分成功三种（不存在/已结束/三段齐全定序）、
  已结束段带进程名、全失败两种（全不存在/全已结束）、混合更新+
  注册、空列表与 interval 非法整批拒绝（无半注册）。
  全量 **170 passed + 4 skipped**。
