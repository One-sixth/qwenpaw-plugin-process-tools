...
async def process_tools_wait(
    ...
    """前台等待一个或一批后台进程结束（all 语义：全部结束才返回）。适用「background 启动后先干别的，快结束时回来等待进程结束」。与前台 exec 的本质区别：本工具只是等待者——超时只报「仍运行中」，绝不杀进程；被取消也只放弃等待，进程不受影响。已结束进程立即幂等返回终态。detach 脱离进程对本插件不可见，无法用本工具等待（需按 PID 用系统命令自理）。

    Args:
        process_id: 进程编号：单个（1 / "1" / "#1"）或编号列表（如 [1, 2]），列表时全部结束才返回（all 语义）。
        timeout: 总等待预算秒数（对整批共享），0 或负数表示一直等到全部结束。超时返回「仍运行中」，进程不受影响，可再次 wait 或挂 process_tools_notice 等异步通知。
        tail_lines: 每个进程返回的日志尾部行数，默认 20，最大 200；0 = 只回状态行。
    """
